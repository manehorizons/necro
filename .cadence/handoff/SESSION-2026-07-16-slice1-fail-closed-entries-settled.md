...
- Working tree is **dirty** and **uncommitted** — this session never commits without being asked. `.cadence/config.json` changes (provider switch to `host-cli`) are mixed into the same uncommitted state as the code changes; if you want to commit the settled phase separately from the provider switch, split them before committing.
...
## Next action
**Action:** Decide what to commit and how to split it: (a) phase 28's code + test + doc changes (the fail-closed entry-resolution feature), and (b) the `.cadence/config.json` provider switch to `host-cli`, are good candidates for two separate commits rather than one. Once committed, run `cadence progress` to confirm the loop is still IDLE, then either pick up a P0 Praxis recommendation (`cadence recommend`) or start Slice 2 of the handoff (`publicApiIds` wiring / export evidence truthfulness) with `cadence draft new --title "..."`.

**Verify:** `git status --porcelain` should go clean after committing; `npm run build && npm run typecheck && npm test` should stay green (428 passed) after any commit split — re-run it if you reorganize the diff.

**If it fails:** if `npm test` regresses after splitting commits, the most likely cause is committing `src/engine/index.ts`'s diagnostics change separately from `src/engine/model.ts`'s `entryResolution` field they both depend on each other — keep phase 28's `src/`/`test/` changes as one atomic commit.
