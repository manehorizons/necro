...
import type { CompetitorBenchResult, CompetitorRunResult, RepoCoverage, SkippedRepo } from "./report.js";
import { predictCases, scorePredictions, type CompetitorPrediction } from "./score.js";
...
export async function runCompetitorBench(cases: EvalCase[], opts: RunCompetitorBenchOpts): Promise<CompetitorBenchResult> {
  ...
  for (const repo of repos) {
    const repoCases = cases.filter((c) => c.provenance?.repo === repo.repo && c.provenance.sha === repo.sha);
    const checkout = await resolveCheckout(repo, opts.cacheDir);
    if (!checkout.ok) {
      skippedRepos.push({ repo: repo.repo, sha: repo.sha, cases: repoCases.length, reason: checkout.reason });
      continue;
    }
    const [knip, tsPrune] = await Promise.all([runKnip(checkout.path), runTsPrune(checkout.path)]);
    ...
    scoredRepos.push({ repo: repo.repo, sha: repo.sha, cases: repoCases.length });
  }

  const tools: CompetitorRunResult[] = [];
  if (knipPredictions.length > 0) {
    tools.push({ tool: "knip", version: knipVersion, metrics: scorePredictions(knipPredictions) });
  }
  if (tsPrunePredictions.length > 0) {
    tools.push({ tool: "ts-prune", version: tsPruneVersion, metrics: scorePredictions(tsPrunePredictions) });
  }
